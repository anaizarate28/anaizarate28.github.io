package com.example.azaratecs360projecttwo;
...
public class database extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addItemBtn.setOnClickListener(v -> {
            ...
            db.addItem(name, Integer.parseInt(qtyStr), desc);
    ...
    private void refreshGrid() {
        ...
        while (c.moveToNext()) {
            ...
            tQty.setOnFocusChangeListener((v, hasFocus) -> {
                if (hasFocus) return;
                try {
                    int newQty = Integer.parseInt(tQty.getText().toString());
                    db.updateItemQuantity(id, newQty);

                    // Alert when quantity is 10 or less
                    if (newQty <= 10) {
            ...
            tDesc.setOnFocusChangeListener((v, hasFocus) -> {
                if (!hasFocus) db.updateItemDescription(id, tDesc.getText().toString());
            });
            ...
            delBtn.setOnClickListener(v -> { db.deleteItem(id); refreshGrid(); });
